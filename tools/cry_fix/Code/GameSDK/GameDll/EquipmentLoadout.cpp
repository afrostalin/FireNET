...
{
	// Loading FireNET loadout
	CryLog(TITLE "Loading multiplayer loadout...");
	const char* m_FireNETLoadout = "Scripts/FireNET/MultiplayerLoadout.xml";
	XmlNodeRef loadoutRoot = gEnv->pSystem->LoadXmlFromFile(m_FireNETLoadout);
	if (loadoutRoot)
	{
		if (!stricmp(loadoutRoot->getTag(), "Loadout"))
		{
			int childCount = loadoutRoot->getChildCount();

			for (int i = 0; i < childCount; ++i)
			{
				XmlNodeRef childXml = loadoutRoot->getChild(i);
				const char *pTag = childXml->getTag();

				// Load characters
				if (!stricmp(pTag, "Character"))
				{
					const char* fileModel = childXml->getAttr("name");
					CryLog(TITLE "Add loadout character (%s)", fileModel);
					AddModel(fileModel);
				}

				if (!stricmp(pTag, "Weapon"))
				{
					const char* fileModel = childXml->getAttr("name");
					CryLog(TITLE "Add loadout weapon (%s)", fileModel);
				}		
			}

			CryLog(TITLE "Multiplayer loadout loaded. All (%d)", childCount);
		}
		else
		{
			CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, TITLE "Failed load FireNET multiplayer loadout. Can't get <Loadout> tag (%s)", m_FireNETLoadout);
		}
	}
	else
	{
		CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, TITLE "Failed load FireNET multiplayer loadout. Can't root (%s)", m_FireNETLoadout);
	}
...
void CEquipmentLoadout::SvSetClientLoadout(int channelId, const CGameRules::EquipmentLoadoutParams &params)
{
	...
	// Load FireNET loadout for player
	auto pLobby = g_pGame->GetGameLobby();
	if (pLobby)
	{
		int firenet_uid = pLobby->GetFireNETuid(channelId);

		if (firenet_uid > 0)
		{
			auto m_Profile = gFireNET->GetProfile(firenet_uid);
			if (m_Profile)
			{
				const char* fileModel = m_Profile->fileModel;			
				CryLog(TITLE "Actor model (%s)", fileModel);

			    uint8 index = GetModelIndexFromName(fileModel);
				CryLog(TITLE "Model index (%d)", index);

				package.m_modelIndex = index;
			}
			else
			{
				CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, TITLE "Set client loadout failed. Can't get profile");
				package.m_modelIndex = params.m_modelIndex;
			}
		}
		else
		{
			CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, TITLE "Set client loadout failed. uid <= 0");
			package.m_modelIndex = params.m_modelIndex;
		}
	}
	else
	{
		CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, TITLE "Set client loadout failed. Can't get game lobby");
		package.m_modelIndex = params.m_modelIndex;
	}
}
//------------------------------------------------------------------------
...
uint8 CEquipmentLoadout::AddModel( const char *pModelName )
{
	...
	if (index == MP_MODEL_INDEX_DEFAULT)
	{
		...
		CryLog(TITLE "Add model (%s) with index (%d)", pModelName, index);
	}
	return index;
}
...
// Get model index for FireNET loadout
uint8 CEquipmentLoadout::GetModelIndexFromName(const char * name)
{
	for (int i = 0; i < m_modelNames.size(); ++i)
	{
		if (!stricmp(m_modelNames[i].c_str(), name))
		{
			return (uint8)i;
		}
	}
