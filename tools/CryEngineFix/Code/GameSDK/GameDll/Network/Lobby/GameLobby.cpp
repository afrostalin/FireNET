...
void CGameLobby::EnterState(ELobbyState prevState, ELobbyState newState)
{
	...
	case eLS_JoinSession:
	{
		...
		// FireNET send uid to server
		if (!m_server)
		{
			SendPacket(eGUPD_FireNET_SetUID);
		}
...
void CGameLobby::SendPacket(GameUserPacketDefinitions packetType, SCryMatchMakingConnectionUID connectionUID /*=CryMatchMakingInvalidConnectionUID*/)
{
    ...
    // Send FireNET SetUID packet
	case eGUPD_FireNET_SetUID:
	{
		const uint32 MaxBufferSize = CryLobbyPacketHeaderSize + CryLobbyPacketConnectionUIDSize + CryLobbyPacketUINT32Size;

		if (packet.CreateWriteBuffer(MaxBufferSize))
		{
			uint32 firenetUID = 0;

			firenetUID = gFireNET->GetUID();

			CryLog(TITLE "Creating packet eGUPD_FireNET_SetUID. FireNET uid = %d", firenetUID);

			SCryMatchMakingConnectionUID localConnection;

			if (m_nameList.Size() > 0)
			{
				SSessionNames::SSessionName &localPlayer = m_nameList.m_sessionNames[0];
				localConnection = localPlayer.m_conId;
			}
			else
			{
				localConnection = CryMatchMakingInvalidConnectionUID;
			}

			packet.StartWrite(packetType, true);
			packet.WriteConnectionUID(localConnection);
			packet.WriteUINT32(firenetUID);
		}
...
void CGameLobby::ReadPacket(SCryLobbyUserPacketData** ppPacketData)
{
    ...
    // Read FireNET SetUID packet
	case eGUPD_FireNET_SetUID:
	{
		CryLog(TITLE "Reading eGUPD_FireNET_SetUID packet");

		auto conId = pPacket->ReadConnectionUID();
		uint32 firenetUID = pPacket->ReadUINT32();

		int playerIndex = m_nameList.Find(conId);
		if (playerIndex == SSessionNames::k_unableToFind)
		{
			CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, TITLE "eGUPD_FireNET_SetUID Packet telling about player who doesn't exist yet, uid=%u", conId.m_uid);
			playerIndex = m_nameList.Insert(CryUserInvalidID, conId, "", NULL, false);
		}

		CryLog(TITLE "Connection uid = %d, profile uid = %d", conId.m_uid, firenetUID);
		m_nameList.m_sessionNames[playerIndex].m_firenet_uid = firenetUID;

		gFireNET->GetProfile(firenetUID);
...
// Get FireNET uid by channel id
int CGameLobby::GetFireNETuid(int channelId)
{
	int firenet_uid = 0;
	...
	if (pPlayer)
		firenet_uid = pPlayer->m_firenet_uid;

	CryLog(TITLE "Get FireNET uid by channel id (%d). FireNET uid = %d", channelId, firenet_uid);
