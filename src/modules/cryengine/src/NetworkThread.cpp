...
bool CNetwork::SendQuery(NetPacket &packet)
{
...
bool CNetwork::SendSyncQuery(NetPacket &packet, float timeout)
{
	// TODO
	return false;
}
...
void CNetwork::onReadyRead()
{
	...
	if (gModuleEnv->pNetWorker)
		gModuleEnv->pNetWorker->ReadPacket(rawmessage.toStdString().c_str());
}
...
bool CNetwork::RegisterGameServer()
{
	...
	packet.WriteInt(net_query_remote_register_server);
	packet.WriteString(gEnv->pConsole->GetCVar("sv_servername")->GetString());   // Server name
	packet.WriteString(gEnv->pConsole->GetCVar("sv_bind")->GetString());         // Server ip
	packet.WriteInt(gEnv->pConsole->GetCVar("sv_port")->GetIVal());              // Server port
	packet.WriteString(mapName.toStdString());                                   // Server map
	packet.WriteString(gEnv->pConsole->GetCVar("sv_gamerules")->GetString());    // Server gamerules
	packet.WriteInt(0);                                                          // Server online
	packet.WriteInt(gEnv->pConsole->GetCVar("sv_maxplayers")->GetIVal());        // Server max players
...
bool CNetwork::UpdateGameServer()
{
	...
	NetPacket packet(net_Query);
	packet.WriteInt(net_query_remote_update_server);
	packet.WriteString(gEnv->pConsole->GetCVar("sv_servername")->GetString());   // Server name
	packet.WriteString(gEnv->pConsole->GetCVar("sv_bind")->GetString());         // Server ip
	packet.WriteInt(gEnv->pConsole->GetCVar("sv_port")->GetIVal());              // Server port
	packet.WriteString(mapName.toStdString());                                   // Server map
