...
TODO("Update clientquerys.cpp - use enum errors!!!")

bool ClientQuerys::ReadPacket(CTcpPacket & packet)
{
	const EFireNetTcpPacketType packetType = packet.getType();
...
bool ClientQuerys::ReadQuery(CTcpPacket & packet)
{
	EFireNetTcpQuery type = packet.ReadQuery();
...
void ClientQuerys::onCreateProfile(CTcpPacket &packet)
{
	...
	if (!(pDataBase->NicknameExists(nickname)))
	{
		...
		if (pDataBase->CreateProfile(m_Client->profile))
		{
			...
			m_paket.WriteStdString(m_Client->profile->nickname.toStdString());
			m_paket.WriteStdString(m_Client->profile->fileModel.toStdString());
			m_paket.WriteInt(m_Client->profile->lvl);
			m_paket.WriteInt(m_Client->profile->xp);
			m_paket.WriteInt(m_Client->profile->money);
			m_paket.WriteStdString(m_Client->profile->items.toStdString());
#ifndef STEAM_SDK_ENABLED
			m_paket.WriteStdString(m_Client->profile->friends.toStdString());
#endif
...
void ClientQuerys::onGetProfile() const
{
	if (m_Client->profile->uid <= 0)
	{
	...
	if (!m_Client->profile->nickname.isEmpty())
	{
		...
		profile.WriteStdString(m_Client->profile->nickname.toStdString());
		profile.WriteStdString(m_Client->profile->fileModel.toStdString());
		profile.WriteInt(m_Client->profile->lvl);
		profile.WriteInt(m_Client->profile->xp);
		profile.WriteInt(m_Client->profile->money);
		profile.WriteStdString(m_Client->profile->items.toStdString());
#ifndef STEAM_SDK_ENABLED
		profile.WriteStdString(m_Client->profile->friends.toStdString());
#endif
...
void ClientQuerys::onBuyItem(CTcpPacket &packet)
{
	...
	if (!m_Client->profile->nickname.isEmpty())
	{
		if (!item.name.isEmpty())
		{
			...
			if (m_Client->profile->lvl < item.minLvl)
			{
...
void ClientQuerys::onInvite(CTcpPacket &packet)
{
	...
	if (inviteType == "friend_invite")
	{
		...
		if (reciverSocket != nullptr)
		{
			...
			invite.WriteStdString(m_Client->profile->nickname.toStdString()); // From
...
void ClientQuerys::onAddFriend(CTcpPacket &packet)
{
	...
	if (pDataBase->ProfileExists(friendUID))
	{
		...
		if (!m_Client->profile->nickname.isEmpty() && friendProfile != nullptr)
		{
			...
			if (m_Client->profile->uid == friendUID) // Block add yourself in friends
			{
			...
			if (UpdateProfile(m_Client->profile) && UpdateProfile(friendProfile))
			{
				...
				{
					CTcpPacket friendPacket(EFireNetTcpPacketType::Result);
					friendPacket.WriteResult(EFireNetTcpResult::AcceptInviteComplete);
					friendPacket.WriteInt(m_Client->profile->uid);
					friendPacket.WriteStdString(m_Client->profile->nickname.toStdString());
					friendPacket.WriteStdString(m_Client->profile->fileModel.toStdString());
					friendPacket.WriteInt(m_Client->profile->lvl);
					friendPacket.WriteInt(m_Client->profile->xp);
					friendPacket.WriteInt(m_Client->profile->money);
					friendPacket.WriteStdString(m_Client->profile->items.toStdString());
					friendPacket.WriteStdString(m_Client->profile->friends.toStdString());

					pServer->sendMessageToClient(friendSocket, friendPacket);
				}
...
void ClientQuerys::onRemoveFriend(CTcpPacket &packet)
{
	...
	if (!m_Client->profile->nickname.isEmpty())
	{
		...
		if (UpdateProfile(m_Client->profile) && UpdateProfile(friendProfile))
		{
			...
			{
				CTcpPacket friendPacket(EFireNetTcpPacketType::Result);
				friendPacket.WriteResult(EFireNetTcpResult::RemoveFriendComplete);
				friendPacket.WriteInt(m_Client->profile->uid);
				friendPacket.WriteStdString(m_Client->profile->nickname.toStdString());
				friendPacket.WriteStdString(m_Client->profile->fileModel.toStdString());
				friendPacket.WriteInt(m_Client->profile->lvl);
...
void ClientQuerys::onChatMessage(CTcpPacket &packet)
{
	...
	{
		if (gEnv->pConsole->GetVariable("bUseGlobalChat").toBool())
		{
			CTcpPacket msg(EFireNetTcpPacketType::ServerMessage);
			msg.WriteServerMessage(EFireNetTcpSMessage::GlobalChatMsg);
			msg.WriteStdString(m_Client->profile->nickname.toStdString());
...
void ClientQuerys::onGetGameServer(CTcpPacket &packet) const
{
	if (m_Client->profile->uid <= 0)
	{
	...
	if (pGameServer != nullptr)
	{
		...
		gameServer.WriteStdString(pGameServer->name.toStdString());
		gameServer.WriteStdString(pGameServer->ip.toStdString());
		gameServer.WriteInt(pGameServer->port);
		gameServer.WriteStdString(pGameServer->map.toStdString());
		gameServer.WriteStdString(pGameServer->gamerules.toStdString());
